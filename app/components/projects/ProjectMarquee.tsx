...
export function ProjectMarquee({ isActive }: ProjectMarqueeProps) {
  ...
  const createRow = (rowIndex: number) => {
    const images = [...projects, ...projects]; // Duplicate for seamless loop
    const duration = [45, 60, 38][rowIndex] || 50;
    const direction = rowIndex % 2 === 0 ? -50 : 50;
    const startX = rowIndex % 2 === 0 ? 0 : -50;
    ...
    return (
      <div
        key={rowIndex}
        ref={(el) => {
          if (el) rowsRef.current[rowIndex] = Array.from(el.children) as HTMLDivElement[];
        }}
